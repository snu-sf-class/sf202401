...
Inductive ceval : com -> state -> option state -> Prop :=
  ...
  | E_DivError : forall st x y z,              (* <----- NEW *)
      (st z = 0) ->
      st =[ x := y / z ]=> None
  | E_Seq : forall c1 c2 st st' ost'',
      ...
      st' =[ c2 ]=> ost'' ->
      st  =[ c1 ; c2 ]=> ost''
  | E_SeqError : forall c1 c2 st,              (* <----- NEW *)
      st  =[ c1 ]=> None ->
      st  =[ c1 ; c2 ]=> None
  ...
  | E_WhileTrue : forall st st' ost'' b c,
      ...
      st  =[ while b do c end ]=> ost''
  | E_WhileError : forall st b c,              (* <----- NEW *)
      beval st b = true ->
      st =[ c ]=> None ->
...
Theorem ceval_deterministic: forall c st ost1 ost2,
     st =[ c ]=> ost1  ->
     st =[ c ]=> ost2 ->
     ost1 = ost2.
Proof.
  ...
  - (* E_Skip *) inversion E2; subst; eauto.
  - (* E_Asgn *) inversion E2; subst; eauto.
  - (* E_Div *) inversion E2; subst; try nia.
    do 2 f_equal. nia.
  - (* E_DivError *) inversion E2; subst; try nia; eauto.
  - (* E_Seq *) inversion E2; subst.
    + apply IHE1_2.
      specialize (IHE1_1 _ H1). inversion IHE1_1; subst. eauto.
    + specialize (IHE1_1 _ H3). inversion IHE1_1.
  - (* E_SeqError *) inversion E2; subst; try nia; eauto.
    specialize (IHE1 _ H1). inversion IHE1.
  - (* E_IfTrue *) inversion E2; subst; try nia; eauto.
    rewrite H in H5. discriminate.
  - (* E_IfFalse *) inversion E2; subst; try nia; eauto.
    rewrite H in H5. discriminate.
  - (* E_WhileFalse *) inversion E2; subst; try nia; eauto.
    + rewrite H in H2. discriminate.
    + rewrite H in H2. discriminate.
  - (* E_WhileTrue *) inversion E2; subst; try nia; eauto.
    + rewrite H in H4. discriminate.
    + specialize (IHE1_1 _ H3). inversion IHE1_1; subst. eauto.
    + specialize (IHE1_1 _ H5). inversion IHE1_1; subst.
  - (* E_WhileError *) inversion E2; subst; try nia; eauto.
    + rewrite H in H4. discriminate.
    + specialize (IHE1 _ H3). inversion IHE1; subst.
Qed.
...
Theorem loop_never_stops : forall st ost',
  ~(st =[ loop ]=> ost').
Proof.
