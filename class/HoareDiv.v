...
From PLF Require Export ImpDiv.
...
Definition ex1 := assert (X = 3).
Definition ex2 : Assertion := True.
Definition ex3 : Assertion := False.

Definition assn1 : Assertion := X <= Y.
Definition assn2 := assert (X = 3 \/ X <= Y).
Definition assn3 : Assertion := assert (Z = ap2 max X Y).
Definition assn4 := assert (Z * Z <= X
                            /\ ~ (((1 + Z) * (ap S Z)) <= X)).
...
Theorem hoare_consequence_post' : forall (P Q Q' : Assertion) c,
  ...
  {{P}} c {{Q}}.
Proof.
  intros. eauto using hoare_consequence_post.
Qed.
