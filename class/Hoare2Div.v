...
From PLF Require Import HoareDiv.
...
Proof.
  ...
  - eexists. split; eauto.
    apply H1.
    + rewrite (div_mod (st Y) (st Z)) at 1; nia.
...
Lemma swap_test_correct: forall (n m: nat),
    {{ X = m /\ Y = n}} swap_test {{ X = n /\ Y = m}}.
Proof.
  unfold swap_test. hauto.
Qed.

Lemma slow_sub_correct: forall (p m: nat),
  {{ True }}
    X := m;
    Z := p;
    while ~(X = 0) do
...
Proof.
  ...
  (*     + eapply (hoare_while (assert (Z - X = p - m ))). *)
  (*       hauto. *)
  (*     + hauto_vc. *)
  (*   - eapply hoare_asgn. *)
  (* } *)
  ...
  hauto.
  - hauto_while (assert (Z - X = p - m)).
  - hauto.
Qed.

Lemma swap'_correct: forall (m n: nat),
  {{ X = m /\ Y = n }}
       X := X + Y;
       Y := X - Y;
       X := X - Y
...
Lemma simple_conditional_correct:
  {{True}}
     if X <= Y then
       Z := Y - X
     else
...
Proof.
  ...
  hauto_while (assert True).
Qed.

Lemma div_correct: forall (m n: nat),
  {{ True }}
       X := m;
       Y := 0;
       while n <= X do
...
Proof.
  hauto.
  - hauto_while (assert (n * Y + X = m)).
  - hauto.
Qed.

Lemma slow_sub'_correct: forall (m n: nat),
    {{ X = m /\ Y = n }}
      while ~(X = 0) do
        Y := Y - 1;
        X := X - 1
...
Proof.
  hauto.
  hauto_while (assert (Y - X = n - m)).
Qed.

Lemma slow_assginment_correct: forall m : nat,
        {{ X = m }}
      Y := 0;
      while ~(X = 0) do
        X := X - 1;
...
Lemma slow_addition_correct: forall n m : nat,
    {{ X = n /\ Z = m }}
      while ~(X = 0) do
         Z := Z + 1;
         X := X - 1
...
Proof.
  hauto.
  hauto_while (assert (X + Z = n + m)).
Qed.

Lemma sqrt'_correct: forall (m: nat),
  {{ X=m }}
    Z := 0;
    while (Z+1)*(Z+1) <= X do
      Z := Z+1
...
Lemma square_correct: forall (m: nat),
  {{ X = m }}
    Y := 0;
    Z := 0;
    while ~(Y = X)  do
...
Proof.
  (* hauto. *)
  (* - hauto_while (assert(X = m /\ Y <= m /\ Z = m * Y)). *)
  (* - hauto. *)
  hauto.
  - hauto_while (assert (Z = m*Y /\ X = m)).
  - hauto.
Qed.

Lemma square'_correct: forall (m: nat),
  {{ X = m }}
    Y := 0;
    Z := 0;
    while ~(Y = X)  do
  ...
  {{ Z = if m <? 100 then m*m else m*m }}.
Proof.
  hauto.
  - hauto_while (assert (Z = X*Y /\ X = m)).
    hauto_vc.
    destruct (st X <? 100); hauto.
  - hauto.
Qed.

Fixpoint factgen (n c: nat) :=
  match c with
  | 0 => 1
  | S c' => n * factgen (n-1) c'         
  end.

Definition fact n := factgen n n.

Lemma factgen_inc_cnt: forall n c,
    factgen n (S c) = factgen n c * (n-c).
Proof.
  intros. revert n. induction c; intros; try (simpl; nia).
  simpl in *. rewrite IHc. nia.
Qed.

Lemma factorial_correct_solution: forall (m: nat),
    {{ X = m }}
  Y := 1 ;
  while ~(X = 0)
  do
...
Proof.
  hauto.
  - hauto_while (fun st => st X <= m /\ st Y = factgen m (m - st X)).
    + hauto_vc.
      replace (m - (st X - 1)) with (S (m - st X)) by nia.
      rewrite factgen_inc_cnt. nia.
    + hauto_vc.
...
Fixpoint power_series (b n: nat) :=
  match n with
  | 0 => 1
  | S n' => b^n + power_series b n'
  end.

Lemma power_series_correct_solution: forall (m: nat),
  {{ True }}
    X := 0;
    Y := 1;
    Z := 1;
...
Proof.
  hauto.
  - hauto_while (fun st => st X = n / pow (st Y) (st Z) /\ st Y = m /\ st X >= 1).
    + hauto_vc.
      rewrite <- div_str_pos_iff in H2 by nia.
      rewrite H1, pow_add_r, pow_1_r at 1.
      rewrite div_div by (try apply pow_nonzero; nia).
      nia.
    + hauto_vc.
      rewrite <- div_str_pos_iff by (try apply pow_nonzero; nia).
      hexploit (mul_succ_div_gt n (st Y ^ st Z)); try (apply pow_nonzero; nia).
      nia.
  - hauto_vc.
    repeat split; eauto using div_1_r; nia.
Qed.
